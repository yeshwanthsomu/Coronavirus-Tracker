...
library(taRifx)
library(readxl)
setwd("~/Desktop/Random Stuff/Daily Cases_2/Coronavirus-Tracker")
...
worldmeter <- read_xlsx("WorldmeterUS.xlsx")
worldmeter$RecoveredRatio[3] <- 0.624594469
worldmeter$RecoveredRatio[43] <- 0.624594469
...
worldmeter <- worldmeter[order(worldmeter$State),]
...
##################################################
plot_ly(confirmed_US_Subset, x =~Date, y =~Active, type = "scatter", mode = "lines") %>%
  ...
  layout(title = paste("Total Active Cases and Chances of Infection for", state, sep=" "),
         ...
         yaxis2 = list(overlaying = "y", side = "right", range=c(0,5), title="Chance of Infection(%)",
